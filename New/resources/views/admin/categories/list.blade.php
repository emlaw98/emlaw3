...
<ul class="nav nav-tabs">
    <li class="nav-item">
        <a class="nav-link active" aria-current="page" href="{{ route('admin.categories.index') }}">Tất cả danh mục</a>
      </li>
    <li class="nav-item">
      <a class="nav-link "  href="{{ route('admin.categories.product') }}">Danh mục sản phẩm</a>
    </li>
    <li class="nav-item">
      <a class="nav-link" href="{{ route('admin.categories.new') }}">Danh mục tin tức</a>
    </li>
  </ul>
...
@if (session('found'))
<div class="alert alert-success">{{ session('found') }}</div>
@endif
<hr>
@if (session('msg'))
...
<table class="table table-bordered">
        <thead>
            <tr>
                ...
                <th width="10%" style="text-align: center">Loại danh mục</th>
        ...
        <tbody>
            @if (!empty($categoryList))
                ...
                <tr>
                    ...
                    <td><a href="{{ route('admin.categories.new',$category->id) }}">{{ $category->name }}</a></td>
                    <td style="text-align: center">{{ $category->updated_at }}</td>
                    <td style="text-align: center">
                        <a  href="{{ route('admin.categories.edit', $category->id) }}" class= "btn btn-warning btn-sm" >Sửa</a>
                    </td>
